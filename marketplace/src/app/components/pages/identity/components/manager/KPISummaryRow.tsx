...
export const KPISummaryRow = ({ terminology: _terminology }: Props) => (
  <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
    {IDENTITY_KPI_CARDS.map((card) => {
      ...
      const sparkData = card.sparkline_7d.map((v: number, i: number) => ({ i, v }));
        ...
        >
          <div className="px-4 pt-4 pb-2">
            ...
            <div className={cn("mt-1.5 flex items-center gap-1 text-[10px] font-semibold",
              ...
              {card.unit === "%" ? `${card.comparison.delta.toFixed(1)}pp` : card.comparison.delta} vs {card.comparison.period?.toLowerCase() ?? "prev"}
            </div>
          </div>
          <div className="px-0">
            <ResponsiveContainer width="100%" height={48}>
              <AreaChart data={sparkData} margin={{ top: 2, right: 0, left: 0, bottom: 0 }}>
                <defs>
                  <linearGradient id={`ident-kpi-${card.id}`} x1="0" y1="0" x2="0" y2="1">
          ...
          <div className="px-4 pb-3">
            <span className="text-[9px] text-neutral-300 font-bold uppercase tracking-wide">7-day trend</span>
          </div>
        </div>
      );
    })}
