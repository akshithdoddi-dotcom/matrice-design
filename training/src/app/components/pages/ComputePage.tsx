...
function ClusterRow({ cluster }: { cluster: Cluster }) {
  ...
  return (
    <div>
      ...
      >
        <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: 2, backgroundColor: TEAL, opacity: hov || open ? 1 : 0, transition: "opacity 100ms ease" }} />
      ...
      {open && (
        <div style={{ position: "sticky", left: 0, zIndex: 1, backgroundColor: "#F8FAFC", borderBottom: "1px solid rgba(0,119,91,0.15)", borderLeft: `3px solid ${TEAL}` }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, paddingTop: 8, paddingBottom: 6, paddingLeft: 31, paddingRight: 16, borderBottom: "1px solid rgba(0,119,91,0.1)" }}>
            ...
            <button onClick={() => setOpen(false)} style={{ marginLeft: "auto", display: "flex", alignItems: "center", justifyContent: "center", width: 20, height: 20, borderRadius: 3, border: "none", background: "transparent", cursor: "pointer", color: "#94A3B8" }}>
          ...
          {cluster.nodes.length === 0 ? (
            ...
            <div style={{ padding: "0 0 8px" }}>
              <div style={{ display: "grid", gridTemplateColumns: SUB_COLS, alignItems: "center", height: 34, paddingLeft: 31, paddingRight: 16, backgroundColor: "rgba(0,0,0,0.035)", borderBottom: "1px solid rgba(0,0,0,0.07)" }}>
                {["Alias", "Status", "Lease Type"].map(h => (
                  <span key={h} style={{ ...HDR, paddingRight: 16 }}>{h}</span>
                ))}
              </div>
              {cluster.nodes.map((node, si) => (
                <div key={node.id} style={{ display: "grid", gridTemplateColumns: SUB_COLS, alignItems: "center", height: 36, paddingLeft: 31, paddingRight: 16, backgroundColor: si % 2 === 1 ? "rgba(0,119,91,0.015)" : "transparent", borderTop: "1px solid rgba(0,119,91,0.06)" }}>
    ...
    <SectionCard>
      <div style={{ overflowX: "auto" }}>
        <div style={{ minWidth: 760 }}>
          <div style={{ display: "flex", alignItems: "center", height: 44, backgroundColor: "#F8FAFC", borderBottom: "1px solid #E2E8F0" }}>
