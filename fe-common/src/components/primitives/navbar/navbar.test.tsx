...
describe("Primitives/Navbar", () => {
  ...
  it("renders notification slot instead of built-in bell", () => {
    render(
      <Navbar
        notificationSlot={<div data-testid="custom-notif">Custom</div>}
        onNotifications={() => {}}
      />,
    );
    expect(screen.getByTestId("custom-notif")).toBeInTheDocument();
    expect(
  ...
  it("renders rightSlot content", () => {
    render(<Navbar rightSlot={<span data-testid="right">Extra</span>} />);
    expect(screen.getByTestId("right")).toBeInTheDocument();
  });

  it("renders data-slot attribute on root", () => {
    const { container } = render(<Navbar />);
    expect(container.querySelector("[data-slot='navbar']")).toBeInTheDocument();
  });
});

describe("Primitives/NavbarDropdownButton", () => {
  const options = [
  ...
  it("calls onChange when an option is selected", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: /option 1/i }));
    fireEvent.click(await screen.findByRole("option", { name: /option 2/i }));
    expect(handler).toHaveBeenCalledWith("opt-2");
  });

  it("renders placeholder when value not found", () => {
    render(
      <NavbarDropdownButton
        value=""
        options={options}
...
describe("Primitives/NavbarActionButton", () => {
...
describe("Primitives/NavbarNotificationButton", () => {
