...
import { DataGrid, DataGridColumn, InterCell } from "../ui/DataGrid";
        ...
        columns={[
          {
            ...
            render: (row) => (
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-full flex items-center justify-center text-[11px] font-bold shrink-0 bg-[#00775B] text-white">
                  {initials(row.name)}
                </div>
                <div>
          ...
          },
          {
            ...
            header: "Role",
            width: "120px",
            render: (row) => <span className={roleBadge(row.role, false)}>{row.role}</span>,
          },
          {
            key: "status",
            header: "Status",
            ...
            render: (row) => (
              <div className="flex items-center gap-1.5">
                <span className={cn("w-2 h-2 rounded-full", row.status === "active" ? "bg-emerald-500" : "bg-gray-300")} />
                <span className="text-[11px] capitalize text-gray-600">{row.status}</span>
              </div>
            ),
          },
          {
            ...
            width: "1fr",
            render: (row, hovered) => (
              <InterCell hovered={hovered} color="#94A3B8" fontSize={11}>{row.lastActive}</InterCell>
            ),
          },
          {
            key: "actions",
          ...
          columns={[
            {
              ...
              width: "140px",
              render: (row) => (
                <span className={cn("text-[11px] font-bold px-2 py-0.5 rounded-full", severityColor[row.label])}>
                  {row.label}
                </span>
              ),
            },
            ...channels.map(ch => ({
              ...
              render: (row: { id: Severity; label: Severity }) => (
                <input
                  type="checkbox"
                  checked={matrix[row.label][ch]}
                  onChange={() => toggleMatrix(row.label, ch)}
                  className="w-4 h-4 cursor-pointer accent-[#00775B]"
                  onClick={e => e.stopPropagation()}
                />
              ),
            })),
          ]}
          data={severities.map(s => ({ id: s, label: s }))}
          compact
        />
      </Card>
    </div>
